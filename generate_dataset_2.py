# вариант с подсчетом количества ответов
import pandas as pd
import random
import statistics
...
def create_dataframe():
    ...
    for i in range(0,10000):
        ...
        result = [count_for_k1, items_EV1.count(0.25), items_EV1.count(0.5), items_EV1.count(0.75), items_EV1.count(1),
                  count_for_k2, items_EV2.count(0.25), items_EV2.count(0.5), items_EV2.count(0.75), items_EV2.count(1), test.count(10)]
        if count_for_k1 == 0:
            k1 = 1
...
test_columns = ["count_requirement_EV1_"+ str(item) for item in range(1,6)] + ["count_requirement_EV2_"+ str(item) for item in range(1,6)] +["count_n_o"]
